...
2. **Copy to `.claude/hooks.py`** in your project root:
   ```bash
   cp .claude/examples/validation.py .claude/hooks.py
   ```
...
def load_policy():
    ...
    with open('.claude/policy.yaml', 'r') as f:
...
**Hooks not loading?**
- Check file location: `.claude/hooks.py` in project root
