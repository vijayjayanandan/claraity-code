...
EDGE_COLORS = {
    "uses": "#58a6ff", "calls": "#3fb950", "writes": "#f85149",
    "reads": "#79c0ff", "spawns": "#bc8cff", "drives": "#f778ba",
    "renders": "#3fb950", "dispatches": "#79c0ff", "controls": "#d29922",
    "configures": "#d29922", "bridges": "#f778ba", "wraps": "#8b949e",
...
def calculate_layout(nodes: list[dict], edges: list[dict]):
    ...
    for mod in modules:
        ...
        mod_layouts.append({
            "id": mod["id"], "node": mod, "label": label,
            "w": w, "h": h, "children": child_layouts,
    ...
    for rank in sorted(rows):
        ...
        for ml in row:
            ml["x"], ml["y"] = cx, cy
            pos_map[ml["id"]] = {"x": ml["x"] + ml["w"] / 2, "y": ml["y"] + ml["h"] / 2, "w": ml["w"], "h": ml["h"]}
            for cl in ml["children"]:
                cl["x"] = ml["x"] + cl["relX"]
                cl["y"] = ml["y"] + cl["relY"]
                pos_map[cl["id"]] = {"x": cl["x"] + cl["w"] / 2, "y": cl["y"] + cl["h"] / 2, "w": cl["w"], "h": cl["h"]}
            cx += ml["w"] + COL_GAP
    ...
    for i, s in enumerate(sys_sorted):
        sys_layouts.append({"id": s["id"], "node": s, "x": sx + i * 136, "y": 10, "w": 120, "h": 28})
...
def build_svg(mod_layouts, sys_layouts, vis_edges, pos_map, total_h) -> str:
    ...
    parts.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" width="{w}" height="{h}" style="max-width:100%;height:auto;">')
    ...
    for e in vis_edges:
        ...
        # find marker id
        marker_id = next((k for k, v in {**EDGE_COLORS, "default": DEFAULT_COLOR}.items() if v == marker_color), "default")
    ...
    for ml in mod_layouts:
        ...
        count_label = f'{ml["count"]} components' if ml["count"] > 0 else f'{ml["file_count"]} files' if ml["file_count"] > 0 else ""
        if count_label:
            parts.append(
...
def build_html(meta: dict, nodes: list[dict], edges: list[dict], svg: str) -> str:
    ...
    for m in modules:
        ...
        comps = [node_map[c] for c in child_ids if c in node_map and node_map[c]["type"] == "component"]
...
<table>
  ...
  <tbody>{''.join(rows_html)}</tbody>
    ...
    for m in modules:
        ...
        out_edges = [
            ed for ed in dep_edges
            if ed["from_id"] == m["id"] or any(
                node_map.get(ed["from_id"], {}).get("id") == cid for cid in child_ids
            )
        ]
        in_edges = [
            ed for ed in dep_edges
        ...
        comp_items = "".join(
            ...
            f'</li>'
            for c in sorted(comps, key=lambda c: (c["properties"].get("flow_rank", 0), c["properties"].get("flow_col", 0)))
        )

        file_item_parts = []
        for fi in sorted(files, key=lambda x: x.get("name", "")):
            fname = e(fi.get("name", ""))
            fpath = f'<span class="file-path">{e(fi["file_path"])}</span>' if fi.get("file_path") else ""
            flines = f'<span class="file-lines">{fi["line_count"]} lines</span>' if fi.get("line_count") else ""
            fdesc = fi.get("description", "")
            fdesc_html = f'<div class="file-desc">{e(fdesc[:150])}{"..." if len(fdesc) > 150 else ""}</div>' if fdesc else ""
            file_item_parts.append(f'<li><div class="file-header"><code>{fname}</code>{fpath}{flines}</div>{fdesc_html}</li>')
...
<details class="mod-detail">
  ...
  <div class="mod-body">
    ...
    meta_line = " &bull; ".join(filter(None, [
        e(lang),
...
def generate(jsonl_path: str = ".claraity/claraity_knowledge.jsonl",
             out_path: str = ".claraity/ARCHITECTURE.html") -> str:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate ARCHITECTURE.html from claraity_knowledge.jsonl")
