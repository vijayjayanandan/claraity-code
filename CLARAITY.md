...
10. **VS Code tool card status has two setters -- both must be correct:** Tool card status in the VS Code webview is set by (1) Python `update_tool_state()` calls in `agent.py` during execution, AND (2) the `STREAM_END` reducer in `reducer.ts` which sweeps any `running`/`pending` cards on stream end. The reducer sweep can OVERRIDE whatever Python emitted. When debugging wrong tool card status, check both layers. The `StreamEnd` event carries `interrupted: bool` to let the reducer distinguish user-stop (→ `cancelled`) from unexpected failure (→ `error`).

11. **VS Code Stop hard-cancels the streaming task:** `StdioServer.submit_action()` does two things on Stop: sets the interrupt flag AND calls `self._streaming_task.cancel()`. The hard cancel raises `CancelledError` before the `check_interrupted()` poll loop in `RunCommandTool` can fire. The tool result becomes an orphan; `_fix_orphaned_tool_calls()` handles it on the NEXT turn. The subprocess is killed correctly, but the result never reaches the agent in the current turn.
